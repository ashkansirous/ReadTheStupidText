...
/// <summary>
/// Exposes the neural (Kokoro) voices to the UI — and only those, by design. The
/// list is empty until the model has downloaded, which the UI reads as a
...
public sealed class NeuralVoiceCatalog : IVoiceCatalog
{
    ...
    public IReadOnlyList<VoiceInfo> InstalledVoices =>
        _model.IsReady ? KokoroVoiceTable.Voices : Array.Empty<VoiceInfo>();

    public VoiceInfo? DefaultVoice => _model.IsReady ? KokoroVoiceTable.Default : null;
}
